...
- 🎯 Entry points to execute your program using `python -m <webchat>` or `$ webchat` with basic CLI argument parsing.
...
---
# webchat Flask Application

Awesome webchat created by Miladshahhi
...
```bash
git clone https://github.com/Miladshahhi/webchat webchat
cd webchat
make install
```
...
```bash
webchat create-db   # run once
webchat populate-db  # run once (optional)
webchat add-user -u admin -p 1234  # ads a user
webchat run
```
