...
original_name="webchat"
original_urlname="webchat"
original_description="Awesome webchat created by Miladshahhi" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/webchat "${name}"
...
then
    ...
    echo "then run 'webchat' or 'python -m webchat'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'webchat' or 'python -m webchat' or 'poetry run webchat'"
fi
